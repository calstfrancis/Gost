...
OUT_DARK="screenshots/gost-main-dark.png"
WINDOW_W=960
WINDOW_H=720
...
# that keyfile never lands in Cal's real ~/.config; Gost resolves its own
...
KEYFILE
  ...
  echo "==> Launching Gost ($scheme) against demo config inside the isolated display"
  env -u WAYLAND_DISPLAY GDK_BACKEND=x11 HOME="$DEMO_HOME" XDG_CONFIG_HOME="$DEMO_HOME/.config" \
    ADW_DISABLE_PORTAL=1 GSETTINGS_BACKEND=keyfile DISPLAY=":$DISPLAY_NUM" python3 -m essay_builder.app &
  APP_PID=$!
...
# (Project dir is "Gost" but the site uses the lowercase slug.)
SLUG="gost"
